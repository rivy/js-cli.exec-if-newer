...
The exec-if-newer application was primarily designed to be used in NPM
...
target files or any concrete/non-glob target files are missing, the command
is executed.
...
The exec-if-newer program is designed to work with [NodeJS](http://nodejs.org) versions >= 6.x.
You must have NodeJS and NPM installed to install and run the application.

### Locally

If you will be installing exec-if-newer for use in NPM scripts for
...
To install exec-if-newer for use across your entire system, use
